...
(function () {
  ...
  const categoryLabels = {
    catcollection: "Havering Museum Collection",
    catplace: "Place",
    catsport: "Sport",
    catpeople: "People",
  };
  ...
  async function loadTimelineHtml() {
    try {
      ...
      // loaded successfully; no notice displayed per request
      // (previously would show count messages in output and categoryOutput)
    } catch (err) {
  ...
  function renderDropdownMenu(matchingCards, yearInput) {
    ...
    let dropdownHtml = `<div class="match-dropdown">
      <strong>Multiple entries found for year ${escapeHtml(yearInput)}. Select one from the dropdown list below:</strong><br><br>
  ...
  function executeSearch() {
    ...
    // Clear old category search results
    if (categoryOutput) {
      categoryOutput.innerHTML = `<div class="notice"></div>`;
    }
  ...
  function searchTimelineByKeyword(keyword) {
    ...
    if (!keyword || keyword.trim() === "") {
      output.innerHTML = `<div class="notice">Please enter a search term.</div>`;
      return;
    }

    if (!parsedDoc) {
      console.log("parsedDoc not loaded yet, loading timeline.html...");
      output.innerHTML = `<div class="notice">timeline.html not loaded yet. Trying again...</div>`;
      loadTimelineHtml().then(() => {
        console.log("timeline.html loaded, retrying search...");
        searchTimelineByKeyword(keyword);
      });
      return;
    }

    console.log("Starting search with normalized keyword");
    const normalizedKeyword = normalizeForSearch(keyword);
    const allCards = Array.from(parsedDoc.querySelectorAll(".card"));
    console.log("Found", allCards.length, "total cards");

    const matchingCards = [];

    // Check if keyword looks like a year (digits only)
    const digits = keyword.replace(/[^0-9]/g, "");
    const isYearSearch = digits && digits.length <= 4;
    console.log("isYearSearch:", isYearSearch, "digits:", digits);

    // Search through all cards for keyword matches
    allCards.forEach((card) => {
      const cardText = normalizeForSearch(card.textContent);
      const cardId = (card.id || "").toLowerCase();

      let isMatch = false;

      // If searching for a year, check if card ID starts with the year pattern
      if (isYearSearch) {
        const first4 = digits.padStart(4, "0");
        if (cardId.match(new RegExp(`^${first4}`))) {
          isMatch = true;
      ...
      // Also check if keyword appears in card content text
      if (!isMatch && cardText.includes(normalizedKeyword)) {
        isMatch = true;
      }
    ...
    renderSearchResults(matchingCards, keyword);
  }
  ...
  function renderSearchResults(matchingCards, searchTerm) {
    ...
    dropdownHtml += `</select>
      <button id="searchSelectBtn">Show Selected</button>
    </div>
    <div id="searchSelectedContent"></div>`;

    output.innerHTML = dropdownHtml;

    const selectElem = document.getElementById("searchMatchSelect");
    const selectBtn = document.getElementById("searchSelectBtn");
    const selectedContent = document.getElementById("searchSelectedContent");
  ...
  // ============ CATEGORY SEARCH FUNCTIONALITY (merged) ============
  ...
  function searchByCategories() {
    ...
    // Also reset the year/timeline output to default (clear any notice)
    if (output) {
      output.innerHTML = ``;
    }
    ...
    if (!selectedCategory) {
      if (categoryOutput) {
        categoryOutput.innerHTML = `<div class="notice">Please select a category.</div>`;
      }
      return;
    }
    ...
    if (!parsedDoc) {
      if (categoryOutput) {
        categoryOutput.innerHTML = `<div class="notice">timeline.html not loaded yet. Trying again...</div>`;
      }
      loadTimelineHtml().then(() => {
  ...
  function performCategorySearch(selectedCategories) {
    const allCards = Array.from(parsedDoc.querySelectorAll(".card"));
    const matchingCards = [];
    ...
    renderCategoryResults(matchingCards, selectedCategories);
  }

  function renderCategoryResults(matchingCards, selectedCategories) {
    if (!categoryOutput) return;

    if (!matchingCards || matchingCards.length === 0) {
      categoryOutput.innerHTML = `<div class="notice">No entries found for the selected category: ${selectedCategories.join(", ")}</div>`;
      return;
    }

    let resultsHtml = `<div class="notice">Found ${matchingCards.length} entries matching: ${selectedCategories.map((cat) => categoryLabels[cat]).join(", ")}</div><br>`;
    ...
    categoryOutput.innerHTML = resultsHtml;
  }

  // Event listeners for timeline search - attach directly with debugging
  console.log("Setting up search listeners...");

  const searchBtnElement = document.getElementById("searchBtn");
  const searchInputElement = document.getElementById("searchQuery");

  console.log("searchBtnElement:", searchBtnElement);
  console.log("searchInputElement:", searchInputElement);
  ...
  if (searchBtnElement) {
    searchBtnElement.onclick = function () {
      ...
      console.log("About to call searchTimelineByKeyword with:", keyword);
      searchTimelineByKeyword(keyword);
    };
    console.log("onclick handler attached to searchBtn");
  } else {
    console.error("searchBtn element not found!");
  }

  if (searchInputElement) {
    searchInputElement.onkeypress = function (e) {
      if (e.key === "Enter") {
    ...
    console.log("onkeypress handler attached to searchInput");
  } else {
